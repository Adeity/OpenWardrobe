...
class WardrobeRepository {
  ...
  Future<List<WardrobeItem>> fetchItems() async {
    ...
    if (isOnline) {
      try {
        final response = await supabaseClient.from('wardrobe_items').select();  // Gebruik get() in plaats van execute()
  ...
  Future<void> addItem(WardrobeItem item) async {
    final isOnline = await _checkConnectivity();
    if (isOnline) {
      await supabaseClient.from('wardrobe_items').insert(item.toJson()).select();  // Gebruik select() om de insert te bevestigen
    } else {
  ...
  Future<void> syncLocalChanges() async {
    final box = await Hive.openBox<WardrobeItem>('wardrobe_items');
    final unsyncedItems = box.values.where((item) => !item.isSynced).toList();

    for (var item in unsyncedItems) {
      await supabaseClient.from('wardrobe_items').insert(item.toJson()).select();
      item.isSynced = true;
